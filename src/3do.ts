...
export function ThreeDO(buffer) {
}
...
function* object(buffer, offset = 0) {
  ...
  yield {name, x, y, z, /*vertex_array, primitive_array*/ }

  if (child_offset) yield* object(buffer, child_offset);
  if (sibling_offset) yield* object(buffer, sibling_offset);

}

function vertexes(buffer, offset, count) : Float32Array {
...
function* primitives(buffer, offset, count) {
  for (let i = 0;i < count;++i) yield primitive(buffer, offset + i * 32);
}
...
function indexes(buffer, offset, count) : Uint16Array {
